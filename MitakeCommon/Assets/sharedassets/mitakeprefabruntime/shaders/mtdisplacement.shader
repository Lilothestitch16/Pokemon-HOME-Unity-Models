Shader "MT/Displacement" {
	Properties {
		...
		_DisplaceMap ("DisplaceMap", 2D) = "black" {}
		[Header(Settings)] _ColorMapUvIndex ("ColorMapUvIndex", Float) = 0
		_Layer1UvIndex ("Layer1UvIndex", Float) = 0
		_DisplaceUvIndex ("DisplaceUvIndex", Float) = 0
		[Toggle(AS_FIRE)] _AsFire ("As Fire", Float) = 0
		...
		_L1ConstantColorSd0 ("L1ConstantColorSd0", Vector) = (1,1,1,1)
		...
		[Header(Displacement)] [KeywordEnum(Sin, Cos, HalfRemap)] _DispOpe ("Displace Operator", Float) = 0
		[KeywordEnum(R, G, B, A)] _VCDispMask ("Displace Mask Channel(Vtx Color)", Float) = 0
		[PowerSlider(3.0)] _DisplaceHeight ("DisplaceHeight", Range(0, 32)) = 1
		_DisplaceBaseU ("DisplaceBaseU", Range(0, 1)) = 0
		_DisplaceBaseV ("DisplaceBaseV", Range(0, 1)) = 0
